...
class SumTree(object):
    ...
    def add(self, p, data): # 将transition放入self.data中
    ...
    def update(self, tree_idx, p): #计算树中各值
...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        b_idx, b_memory, ISWeights = np.empty((n,), dtype=np.int32), np.empty((n, self.tree.data[0].size)), np.empty((n, 1)) # np.empty()生成的数据不为0，是随机产生的数值
        ...
        for i in range(n):
            ...
            idx, p, data = self.tree.get_leaf(v) # idx在树中的index，p为树中该位置的p值，data为self.data对应的transition
